#### A.  Research Question
Cleaned the data set to be able to predict what customers are most likely to churn. The company then could focus on those customers to prevent them from possibly churning. 
 
#### B. Variables
...
I could not find a clear-cut correlation from the column, unfortunately. From the count plot above, I could see that median and mean had a gap between them. The Central limit theorem states that if you grab a samples large enough from the column, you will find a normal distribution histogram. 
...
The Income column has a normal distribution column so the method I would use to identify the outliers are standard deviations * 3.
...
#### Summarize the outcome from the implementation of _each_ data-cleaning step.
After each cleaning step in the data, we can see from the PCA that there are two clusters of groups. The blue represents the customer who did not churn, and the green is the customers who churned the company.  I will explain the PCA in more detail later in the document. 
![[Pasted image 20210427180426.png]]
...
The organization can use four independent variables instead of seven dependable variables. It will reduce the dimensionality of the data and only use the essential variations of the data.  Finding the most critical variables is the core of building a predictive model when we get to that part of the project.
![[Pasted image 20210427173440.png]]
For example, You can see from the visualization above that PC0 tells us that Tenure and Bandwidth_GB have some correlation. PC1 tells us that Outage sec per week and monthly charge have some correlation and so on. Knowing what columns have correlations will help the project in the exploration phase of the project.
