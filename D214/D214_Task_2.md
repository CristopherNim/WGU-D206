...
# Nursing Home Random Forest Model Research
...
I am extracting data and preparing data to predict the number of fines features. Looking at the spreadsheet, I noticed that the total amount of fines in dollars would increase every time a fine occurs. In addition, a total number of penalties will essentially match the number of fines columns. If we left both features, our model would produce a 95% accuracy rate.
![[Pasted image 20210920145154.png]]

Once we remove these two features, we get a 79% accuracy rate to predict whether the nursing home will be fine.
...
The model I will use to predict the fined feature is random forest. I will outline the assumptions, advantages, and disadvantages.
##### Random Forest Assumptions
The only assumption of the random forest model is that it can handle outliers and multi-modal data. 
...
- Biased when dealing with categorical variables. 
	- The categorical feature columns will have some biased attached to them

###### Preparing the data for the Random Forest Model
1) The y variable will represent whether the nursing home was fined or not. The X features are going to be used to predict the y feature. 
2) I then split the X variable into a training and testing set. I do the same for the y variable. Furthermore, the test sizes will be 20%, and 80% of the data will be used for the training sets.  
 ![[Pasted image 20210920193147.png]]
4) We train the data to create a forest. 
![[Pasted image 20210920193244.png]]
6) Finally, We test the data. The model score is 79%.
 ![[Pasted image 20210920193627.png]]

##### Feature importance
![[Pasted image 20210922140149.png]]
The graph above lets us visualize what the essential features are.

##### chi-square 
- The main disadvantage of using chi-square is that random forest model sometimes produces biased towards categorical features. The main advantage that it allows us to remove the features that do not aid in the prediction of number of fine features.
1) The first to test the dataset using the chi-square is to grab all the categorical columns. The code below grabs only the categorical features.                                          ![[Pasted image 20210922140514.png]]
2) The code below tests a relationship between the number of fines and all the other categorical features.
![[Pasted image 20210922140712.png]]
3) The accepted list has features that do have a relationship with the number of fines feature. The rejected list has a feature that does not have a relationship with a number of fines feature. 
...
The model score is 78% after removing the features that do not aid in predicting the number of fine features. 
![[Pasted image 20210922141512.png]]
The importance feature graph looks similar, with the only difference being removing the rejected columns. 
#### we now only have 58 features after starting with 87 features 

#### Confusion matrix
- Confusion matrix advantage allows to see what prediction it had right and what prediction it had wrong. The results are not always reproducible if we grab different samples of training test sets. 
![[Pasted image 20210922141802.png]]

##### Area Under the Curve (AUC)
The model Area under the curve is 71%. Meaning that is labeling the correct label 71% of the time. The main disadvantage is that if it testing set samples are different it would produce a different accuracy score. 
...
- The Chi-square test allowed us to reduce the number of features used to predict the feature.  We started with 89 columns, including the feature we are predicting. We then used the chi-square test to remove the categorical features that do not aid in the prediction. After removing the categorical columns that did not have a relationship, we were left with only 57 columns. The disadvantage is that accuracy lower by 1 percent in the sample after removing features using the chi-square. Is it worth removing the features when the data is easy to collect, or will it be collected regardless?
 - The top feature that aids in the prediction is the health survey score. The feature that least helps is 'most_recent_health_inspection_more_than_2_years_ago'. We could remove the column to help lower the feature if collecting this specific data is difficult. 
 ...
 - The limitation for random forest classifier is that if it has many trees, the algorithm is slow and ineffective for right away use. Waiting to get the results might cost the company money.
 - The main limitation is that I do not have an expert on the data that could verify if the features were correctly collected and if other columns are unnecessary. 
#### Recommended course of action 
- The model had an accuracy of 78%, which is 8% more than the goal. The recommended course of action is that each nursing home should collect accurate data on the most important features to help aid in the prediction of fines. Furthermore, nursing should improve the rating cycle scores to help decrease the chance of getting fined. If the other features are not essential, they should not be collected unless the state needs to help collect the other important features. 

#### Direction and approaches for future studies of the data set
- Instead of building a Random Forest model, we could use a naive Bayes model. With naive bayes model numerical features would have to be normalized and outliers will have to be removed for an accurate model.
- Reaching out to different nursing homes to verify if the data collected is accurate and if they believe the most critical features are reasons why the nursing home gets fined. Furthermore, ask those nursing homes if they believe other features can help aid in the prediction. 
- Another approach is removing the rows of the nursing homes that were affected due to covid-19. The nursing home might not have been in danger of getting fined before covid-19.
